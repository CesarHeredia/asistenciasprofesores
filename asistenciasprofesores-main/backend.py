...
def get_dashboard_data(db_path):
    ...
    try:
        ...
        estado_profesores = {}
        asistencia_hoy = {}   # hora_entrada y hora_salida del día para cada profesor
        for r in attendance_rows:
            ...
            # Guardar los tiempos reales (el último registro del día sobreescribe, que es el comportamiento correcto)
            asistencia_hoy[pid] = {
                "hora_entrada": h_in or None,
                "hora_salida":  (h_out if h_out and h_out.strip() else None)
            }
        ...
        for id_prof, info in profesores_dict.items():
            ...
            asist_info = asistencia_hoy.get(id_prof, {})
            result.append({
                "id_profesor": id_prof,
                "nombre": info["nombre"],
                "materia": materia_str,
                ...
                "hora_entrada_hoy": asist_info.get("hora_entrada"),
                "hora_salida_hoy":  asist_info.get("hora_salida")
            })
...
def get_attendance_history(db_path):
    ...
    Cada registro incluye 'ultimo_fin_horario': la hora_fin más tardía de los bloques del profesor
    para el día de la semana correspondiente a la fecha del registro, usada para calcular el estado."""
    import datetime
    if not os.path.exists(db_path):
        return []
    try:
        ...
        # Map day number (weekday()) to Spanish day names used in the horario table
        dias_map = {0: "Lunes", 1: "Martes", 2: "Miércoles", 3: "Jueves", 4: "Viernes", 5: "Sábado", 6: "Domingo"}

        for row in rows:
            # Determine the day of week for the attendance record's date
            try:
                fecha = datetime.date.fromisoformat(row["fecha_asistencia"])
                dia_semana = dias_map.get(fecha.weekday(), "")
            except Exception:
                dia_semana = ""

            # Fetch the latest hora_fin the professor has on that day of the week
            if dia_semana:
                cur.execute("""
                    SELECT MAX(hora_fin) as ultimo_fin
                    FROM horario
                    WHERE id_profesor = ? AND dia_semana = ?
                """, (row["id_profesor"], dia_semana))
                sched_row = cur.fetchone()
                row["ultimo_fin_horario"] = sched_row["ultimo_fin"] if sched_row and sched_row["ultimo_fin"] else None
            else:
                row["ultimo_fin_horario"] = None
